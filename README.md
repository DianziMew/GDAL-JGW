...
TIFF 批量转为高压缩比的 JPG
自动提取 TIFF 内部的地理参考信息，并生成对应的 .jgw (World File)，确保图片在 ArcGIS/QGIS 中依然能精准“定位”
核心特性
1.一键处理整个文件夹，告别手动导出。
2.通过生成 .jgw 文件，完美保留地理位置信息。
3.自动处理 4 波段影像带来的“颜色变异”（CMYK 错误），强制回归标准 RGB。
4.智能识别并跳过 .ovr、.aux 等冗余辅助文件。

# 🌍 Geo-Tiff2JPG: 地理影像轻量化搬运工
